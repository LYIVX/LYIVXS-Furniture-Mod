...
public class SimpleFluidRenderer
{
    public static void drawContainer(Level level, BlockPos pos, LiquidHolderBlockEntity blockEntity, AABB box, PoseStack poseStack, MultiBufferSource source, int light)
    {
        ...
        // Top Face
        consumer.vertex(matrix, (float) box.minX, offset, (float) box.minZ).color(red, green, blue, alpha).uv(u0, v0).uv2(light).normal(normalMatrix, 0, 1, 0).endVertex();
        consumer.vertex(matrix, (float) box.minX, offset, (float) box.maxZ).color(red, green, blue, alpha).uv(u0, v1).uv2(light).normal(normalMatrix, 0, 1, 0).endVertex();
        consumer.vertex(matrix, (float) box.maxX, offset, (float) box.maxZ).color(red, green, blue, alpha).uv(u1, v1).uv2(light).normal(normalMatrix, 0, 1, 0).endVertex();
        consumer.vertex(matrix, (float) box.maxX, offset, (float) box.minZ).color(red, green, blue, alpha).uv(u1, v0).uv2(light).normal(normalMatrix, 0, 1, 0).endVertex();
    }
