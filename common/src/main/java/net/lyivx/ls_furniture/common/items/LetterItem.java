...
public class LetterItem extends Item {
    ...
    public void appendHoverText(ItemStack itemStack, TooltipContext tooltipContext, List<Component> list, TooltipFlag tooltipFlag) {
        ...
        if(itemStack.has(DataComponents.CONTAINER)) {
            ...
            list.add(Component.translatable("tooltip.ls_furniture.letter.attachment", attachment.getItem().getDescriptionId()).withStyle(ChatFormatting.GRAY));
        }
    }
        ...
        if(playerEntity.isCrouching()) {
            if(hand != InteractionHand.OFF_HAND) {
                ...
                if(!offHandStack.isEmpty()) {
                    ...
                    if(result.isEmpty()) {
                        if(world.isClientSide()) {
                            playerEntity.displayClientMessage(Component.translatable("msg.ls_furniture.letter.attachment_added", offHandStack.getItem().getDescriptionId()), true);
